    ...
    <>
      ...
      <section className="section">
        <div className="container-prose grid items-start gap-10 md:grid-cols-[2fr_1fr]">
          <div className="space-y-6">
            <QuickAnswer>
              We are a Bali-based air conditioning company specialising in residential and commercial AC installation, cleaning, servicing and 24/7 repair. We are authorised partners for Gree and Daikin. A standard 1 PK split installation costs from IDR 850,000, a chemical wash from IDR 250,000 per unit, and we cover Canggu, Seminyak, Uluwatu, Ubud, Sanur and 35+ other Bali areas same-day.
            </QuickAnswer>
      ...
      <section className="section bg-brand-mist">
        <div className="container-prose">
          ...
          <p className="mt-2 max-w-2xl text-slate-700">From a single bedroom split clean to a beach-club VRF design — we handle every AC job in Bali, with fixed pricing and a 12-month workmanship warranty.</p>
      ...
      <section className="section">
        <div className="container-prose">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <USP icon={<ShieldCheck size={22} aria-hidden />} title="Fixed-price quotes" body="No hourly meter running. Every job quoted upfront on WhatsApp before we send a tech." />
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container-prose">
          <h2 className="h2">Pricing transparency</h2>
          <p className="mt-2 max-w-2xl text-slate-700">Starting prices for the most common jobs in Bali. We send a fixed total on WhatsApp before any visit.</p>
          <div className="mt-6 overflow-x-auto rounded-2xl border border-slate-100">
            <table className="min-w-full divide-y divide-slate-100 text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {services.map((s) => (
                  <tr key={s.slug}>
                    <td className="px-4 py-3 font-medium"><Link className="hover:text-brand" href={`/services/${s.slug}`}>{s.shortName}</Link></td>
                    <td className="px-4 py-3 text-brand font-semibold">{formatIDR(s.startingPrice)}</td>
                    <td className="px-4 py-3 text-slate-600">{s.startingPriceLabel}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="section bg-brand-mist">
        <div className="container-prose">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            ...
            <Testimonial name="Made, Seminyak café manager" body="Two cassettes installed overnight without us closing for service. Quiet, clean install, fair price." />
      ...
      <section className="section">
        <div className="container-prose">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            <BlogTeaser slug="daikin-vs-gree-bali" title="Daikin vs Gree in Bali: Which AC Brand Is Best?" excerpt="Side-by-side: warranty, noise, price and tropical durability — which one wins for Bali villas." />
            <BlogTeaser slug="ac-installation-cost-bali-2026" title="AC Installation Cost in Bali 2026" excerpt="Full breakdown — supply, labour, pipe, gas — for split, multi-split and cassette systems." />
